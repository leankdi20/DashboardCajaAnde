# apps/dashboard/reports/soli_seguro_viajero.py

from ..services.db_service import ReportesDBService

VISTA = "dbo.vw_Solicitud_seguro_viajero"


class ReporteSolicitudSeguroViajero:

    QUERY = f"""
        SELECT
            respuesta_id, FechaHora, Cedula, NombreCompleto, Correo, Telefono,
            Destino, FechaInicioViaje, FechaFinalViaje,
            NombreBeneficiario, CedulaBeneficiario, Parentesco
        FROM {VISTA}
    ...
    QUERY_KPIS = f"""
        SELECT
            ...
            COUNT(DISTINCT RTRIM(Destino))                                         AS total_destinos,
            SUM(CASE WHEN NombreBeneficiario IS NOT NULL THEN 1 ELSE 0 END)        AS total_con_beneficiario,
    ...
    QUERY_POR_DESTINO = f"""
        SELECT TOP 10 RTRIM(Destino) AS Destino, COUNT(*) AS total
        FROM {VISTA}
        WHERE Destino IS NOT NULL
        GROUP BY RTRIM(Destino)
        ORDER BY total DESC
    """
    ...
    def _construir_filtros(filtros: dict):
        ...
        if filtros.get("destino"):
            condiciones.append("AND RTRIM(Destino) = %s")
            params.append(filtros["destino"].strip())
    ...
    @classmethod
    def obtener_top_destinos(cls):
        return ReportesDBService.ejecutar_query(cls.QUERY_POR_DESTINO)
