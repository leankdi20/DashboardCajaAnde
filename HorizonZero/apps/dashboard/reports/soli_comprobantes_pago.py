# apps/dashboard/reports/comprobantes_pago.py

from ..services.db_service import ReportesDBService

VISTA = "dbo.vw_comprobantes_pago"


class ReporteComprobantesPago:

    QUERY = f"""
        SELECT
            respuesta_id, Fecha, Hora, Cedula, NombreCompleto, NumeroTelefonico,
            Banco, NumeroDeposito, FechaDeposito, Monto, DetallePago,
    ...
    QUERY_KPIS = f"""
        SELECT
            ...
            SUM(CASE WHEN Monto IS NOT NULL THEN 1 ELSE 0 END)             AS total_con_monto,
            AVG(CASE WHEN Monto IS NOT NULL
                     THEN TRY_CAST(Monto AS FLOAT) END)                    AS promedio_monto,
            SUM(CASE WHEN TRY_CAST(Monto AS FLOAT) IS NOT NULL
                     THEN TRY_CAST(Monto AS FLOAT) ELSE 0 END)             AS monto_total,
            SUM(CASE WHEN Documento1 IS NOT NULL THEN 1 ELSE 0 END)        AS total_con_doc
        FROM {VISTA}
        WHERE 1=1
        {{filtros_base}}
    """
    ...
    def _construir_filtros(filtros: dict):
        ...
        if filtros.get("banco"):
            condiciones.append("AND RTRIM(Banco) = %s")
            params.append(filtros["banco"].strip())
